...
def receptionist_register():
    ...
    while True:
        ...
        if option_picked == '1':
            receptionist_register_new_guest()
            return
...
def receptionist_register_new_guest():
    print()
...
def receptionist_update_guest():
    ...
    while True:
        try:
            temp_index = input("Please enter the index of the Guest you want to edit (type: back to go back to menu): ")
            if temp_index.lower() == 'back':
                receptionist_manage_booking()
                return
            else:
        ...
        if 0 <= temp_index < len(guest_data_List):
            break
        print(f"\033[31mIndex must be between 1 and {len(guest_data_List)}\033[0m")
    
    while True:
        try:
            print("")
            print("\033[34mWhat do you want to update?\033[00m")
            print(f"1.Name (Currently: {guest_data_List[temp_index][0]})")
            print(f"2.Phone Number (Currently: {guest_data_List[temp_index][1]})")
            print(f"3.Email (Currently: {guest_data_List[temp_index][2]})")
            print(f"4.Address (Currently: {guest_data_List[temp_index][3]})")
    ...
    guest_data_List[temp_index][option-1] = str(x)
    from_array_to_txt_file_conversion(guest_data_List,"guest.txt",'w')
    print(f"\033[32mSuccessfully Updated a Guest index {temp_index + 1}\033[00m")
    receptionist_register()
...
def receptionist_manage_booking():
    ...
    while option_picked not in ['1', '2', '3', '4']: #Checking if the input is valid or not
        print(" ") #spacing reasons
        print("\033[33mManaging Rooms\033[0m")
        print("\033[33mThese are the current Rooms\033[0m")
        #Printing in a nice table format
        print("Index".ljust(10),end="")
        for col in header:
